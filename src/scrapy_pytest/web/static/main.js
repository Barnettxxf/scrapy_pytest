    ...
    $('select').on('change', function (e) {
        ...
        superagent.get(`/filter?storage=${storage}&spider=${spider}`).end(function (err, data) {
            ...
            for (let row of data.body.rows) {
                str = str + `<tr><td><input type="checkbox"></td><td>${row.id}</td><td>${row.storage}</td><td>${row.spider}</td><td>${row.parse_func}</td><td>${row.url}</td><td>${JSON.stringify(row.meta)}</td><td><a href="/del?request_id=${row.id}">delete</a></td></td></tr>`;
            }
            console.log(str);
            $('#content').html(str);
            $('.pagination').html('');
